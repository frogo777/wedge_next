...
export function documentEntries(documents = []) {
  ...
  return [...groups.values()].map(group => {
    const d = group.find(d => ['read', 'review'].includes(d.status)) || group[0];
    const m = d.metadata;
    const conflict = group.some(d => d.status === 'conflict') || new Set(group.map(d => d.fingerprint)).size > 1;
    const supported = ['read', 'review'].includes(d.status) && m.currency === 'MXN' && m.type === 'I' && /^\d{1,12}\.\d{2}$/.test(m.total);
...
export function collectionTransition(documents, collections, event, now) {
  const entry = documentEntries(documents).find(d => d.sampleId === event.id);
...
export function monthlyLedger(documents, collections, period) {
