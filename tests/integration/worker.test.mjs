...
test('Worker: acceso protegido, cabeceras y recursos desplegables', { timeout: 10000 }, async () => {
  ...
  for (const path of ['/app.mjs', '/workflow.mjs', '/document-view.mjs', '/calculator-view.mjs', '/fiscal/resico-isr.mjs', '/style.css', '/favicon.svg']) {
...
test('Worker: cierre y lectura XML se guardan y exportan por cuenta', { timeout: 10000 }, async () => {
  ...
  assert.equal(exported.record.userId, 'journey-a'); assert.equal(exported.record.documents.length, 1);
});
