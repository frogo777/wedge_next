...
const SELECT='SELECT stage,resolved,version,updated_at FROM demo_progress WHERE user_id = ?';
function decode(row){
 if(!row)return {...initialState(),version:0,updatedAt:null};
 ...
 return {stage:row.stage,resolved,version:row.version,updatedAt:row.updated_at};
}
export async function handleProgress(request,db){
 ...
 try{
  if(request.method==='GET'){
   const state=decode(await db.prepare(SELECT).bind(userId).first());
  ...
  if(!body||typeof body!=='object'||Array.isArray(body)||Object.keys(body).some(k=>!['event','version'].includes(k))||!Number.isSafeInteger(body.version)||body.version<0)return json({error:'invalid_payload'},400);
  const e=body.event;
  if(!e||typeof e!=='object'||Array.isArray(e)||Object.keys(e).some(k=>!['type','id'].includes(k))||!['RESOLVE','REVIEW','APPROVE','FILE','PAY','RESET'].includes(e.type)|| (e.type==='RESOLVE'?!taskIds.includes(e.id):e.id!==undefined))return json({error:'invalid_event'},400);
  const row=await db.prepare(SELECT).bind(userId).first();const state=decode(row);
  if(state.version!==body.version)return json({error:'version_conflict'},409);
  ...
  const updatedAt=new Date().toISOString();const version=state.version+1;
  const statement=row?
   db.prepare('UPDATE demo_progress SET stage=?,resolved=?,version=?,updated_at=? WHERE user_id=? AND version=?').bind(next.stage,JSON.stringify(next.resolved),version,updatedAt,userId,state.version):
   db.prepare('INSERT INTO demo_progress (user_id,stage,resolved,version,updated_at) VALUES (?,?,?,?,?) ON CONFLICT(user_id) DO NOTHING').bind(userId,next.stage,JSON.stringify(next.resolved),version,updatedAt);
  const result=await statement.run();if(result.meta.changes!==1)return json({error:'version_conflict'},409);
  return json({state:{...next,version,updatedAt}});
 }catch{
