...
const SELECT='SELECT stage,resolved,version,updated_at,revision,documents FROM demo_progress WHERE user_id = ?';
function decode(row){
 if(!row)return {...initialState(),version:0,updatedAt:null,revision:null,documents:[]};
 ...
 return {stage:row.stage,resolved,version:row.version,updatedAt:row.updated_at,revision:row.revision,documents};
}
export async function handleProgress(request,db){
 ...
 try{
  ...
  const e=body.event;
  if(!e||typeof e!=='object'||Array.isArray(e)||Object.keys(e).some(k=>!['type','id'].includes(k))||!['RESOLVE','REVIEW','APPROVE','FILE','PAY','RESET','ERASE','ADD_DOCUMENT'].includes(e.type)|| (e.type==='RESOLVE'?!taskIds.includes(e.id):e.type==='ADD_DOCUMENT'?!sampleIds.includes(e.id):e.id!==undefined))return json({error:'invalid_event'},400);
  ...
  let documents=state.documents;let documentResult;
  ...
  const next=e.type==='ADD_DOCUMENT'?{stage:state.stage,resolved:state.resolved}:transition({stage:state.stage,resolved:state.resolved},e);
  const unchanged=documents===state.documents&&next.stage===state.stage&&JSON.stringify(next.resolved)===JSON.stringify(state.resolved);
  ...
  const statement=row?
   db.prepare('UPDATE demo_progress SET stage=?,resolved=?,version=?,updated_at=?,revision=?,documents=? WHERE user_id=? AND version=? AND revision=?').bind(next.stage,JSON.stringify(next.resolved),version,updatedAt,revision,JSON.stringify(documents),userId,state.version,state.revision):
   db.prepare('INSERT INTO demo_progress (user_id,stage,resolved,version,updated_at,revision,documents) VALUES (?,?,?,?,?,?,?) ON CONFLICT(user_id) DO NOTHING').bind(userId,next.stage,JSON.stringify(next.resolved),version,updatedAt,revision,JSON.stringify(documents));
  const result=await statement.run();if(result.meta.changes!==1)return json({error:'version_conflict'},409);
  return json({state:{...next,version,updatedAt,revision,documents},...(documentResult?{documentResult}:{} )});
 }catch{
